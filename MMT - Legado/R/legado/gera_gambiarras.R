...
write.xlsx(cnae_bienio, "corridamaluca.xlsx")

cnae_bienio = read_excel("P:/RECENTES/DIEPS/GEFAPS/GEFAPS/2022/Panorama da Capacitação/rais-panorama/corridamaluca.xlsx", sheet = "Sheet1")
...
proporcoes <- dados %>% group_by(referencia, tipo_educ) %>% summarise(vinculos = sum(vinculoativo3112))

write_xlsx(proporcoes, "proporcoes.xlsx")

proporcoes2 <- dados %>% filter(idade %in% 18:27) %>% group_by(referencia, tipo_educ) %>% summarise(vinculos = sum(vinculoativo3112))

write_xlsx(proporcoes2, "proporcoes2.xlsx")
